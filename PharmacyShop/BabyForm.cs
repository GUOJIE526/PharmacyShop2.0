    ...
    public partial class BabyForm : Form
    {
        ...
        void showProdDesc()
        {
            if (lsvBaby.SelectedItems.Count > 0)
            {
                ...
                try
                {
                    string strDB = "select * from baby where id = @id";
                    SqlCommand cmd = new SqlCommand(strDB, conn);
                    cmd.Parameters.AddWithValue("@id", lsvBaby.SelectedItems[0].Tag);
        ...
        private void btnAddCart_Click(object sender, EventArgs e)
        {
            ...
            if (lsvBaby.SelectedItems.Count > 0)
            {
                ...
                try
                {
                    string strDB = "update baby set qty = @qty where id = @id";
                    SqlCommand cmd = new SqlCommand(strDB, conn);
                    cmd.Parameters.AddWithValue("@qty", lastqty);
                    cmd.Parameters.AddWithValue("@id", lsvBaby.SelectedItems[0].Tag);
                    Console.WriteLine(lsvBaby.SelectedItems[0].Tag);
        ...
        private void lsvBaby_ItemActivate(object sender, EventArgs e)
        {
            showProdDesc();
        }
    }
